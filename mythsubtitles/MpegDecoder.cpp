...
MpegDecoder::decodeVideo()
{
    ...
    if (gotPic)
    {
        if ((verbose > 1 && pic->atsc_cc_len > 0) || pic->dvb_cc_len > 0)
            fprintf(fplog, "pos=%lld atsc_cc_len=%d dvb_cc_len=%d\n",
                (long long)pkt.pos, pic->atsc_cc_len, pic->dvb_cc_len);
        for (uint i = 0; i < (uint)pic->atsc_cc_len;
                 i += ((pic->atsc_cc_buf[i] & 0x1f) * 3) + 2)
        {
            DecodeDTVCC(pic->atsc_cc_buf + i);
        }
        if (pic->dvb_cc_len > 0)
...
void
MpegDecoder::DecodeDTVCC(const uint8_t *buf)
{
#if 0
    // closed caption data
    //cc_data() {
    ...
    {
        if (verbose > 1)
            fprintf(fplog, "process_cc_data_flag=0\n");
        return;
    }
    uint cc_count = *buf & 0x1f;
    if (verbose > 1)
        fprintf(fplog, "process_cc_data_flag=%#x cc_count=%u\n", *buf, cc_count);
    buf += 2;
    ...
    for (uint i = 0; i < cc_count; i++, buf += 3)
    {
        if (!(*buf & 0x04))
        {
            if (verbose > 2)
                fprintf(fplog, "cc_code=%#x\n", *buf);
            continue;
        }
        if (verbose > 1 && (*buf & 0x02) == 0)
        {
            fprintf(fplog, "cc_code=%#x", *buf);
            logBytes(buf+1, 2);
        }

        switch (*buf & 0x3)
        {
            case 0: // EIA-608 field 1, line 21
                sliced->line = 21;
                sliced->id = VBI_SLICED_CAPTION_525;
                memcpy(sliced->data, buf+1, 2);
                sliced++;
                break;
            case 1: // EIA-608 field 2, line 284
                sliced->line = 284;
                sliced->id = VBI_SLICED_CAPTION_525;
                memcpy(sliced->data, buf+1, 2);
                sliced++;
                break;
            default: // EIA-708 not implemented
                break;
        }
    }
